                    ...
                    <div class="col-lg-9">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <button type="button" class="btn border-none outline-none" data-bs-toggle="modal"
...
include '../conn.php';
    ...
    if (isset($_POST['add_package'])) {
        ...
        $pack_img_err = $file['error'];
        $pack_tmp = $file['tmp_name'];
        $pack_name = $file['name'];


        if ($pack_img_err == 0) {
            $pack_path = './assets/package_theme/'.$pack_name;
            $upload_img = move_uploaded_file($pack_tmp, $pack_path);
            if ($upload_img) {

                $insert = "INSERT INTO `package_theme`(`package_theme`, `name`) VALUES ('$pack_path','$title')";

                $insert_run = mysqli_query($conn, $insert);

                if ($insert_run) {
                    echo "<script>alert('Add New Package Theme successfully !');</script>";

                } else {
                    echo "<script>alert('OOps New Package Theme not successfully !')</script>";
                }
            }
